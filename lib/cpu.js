	...
	step: function() {
		...
		switch(cpu.state) {
			...
			case 112:
				...
				break;
			case 113:
				setState(0, "Execute", "The *opcode* and *Control Unit* sends the value in the *Memory Data Register* to the *Accumulator* register.");
				cpu.registers.acc = cpu.registers.mdr;
				cpu.updateValues();
				$('.active').removeClass('active');
			...
			case 12://The *opcode* 0110 means branch (unconditionally)
				setState(0, "Execute", "The *operand* gets stored in the *Program Counter*");
			...
			case 13://The *opcode* 0111 means branch if the *Accumulator* stores a value equal to 0
				setState(0, "Execute", "The *Control Unit* and *opcode* makes the *Arithmetic Logic Unit* check to see if the *Accumulator* register contains a zero. If it does, the *operand* gets copied into the *Program Counter* register");
			...
			case 14://The *opcode* 1000 means branch if the *Accumulator* stores a value greater than or equal to 0"
				setState(0, "Execute", "The *Control Unit* and *opcode* makes the *Arithmetic Logic Unit* check to see if the *Accumulator* register contains a value greater than zero. If it does, the *operand* gets copied into the *Program Counter* register");
				if(cpu.registers.acc >= 0)
					cpu.registers.pc = cpu.registers.cir & 0x0F;
				$('.active').removeClass('active');
				cpu.updateValues();
			...
			case 15://The *opcode* 1001 means either input or output, depending on the *operand*"
				if((cpu.registers.cir & 0x0F) == 1) {
					setState(0, "Execute", "The *opcode* 0001 and the *Control Bus* reads from the input device and places the input value into the *Accumulator* register");
				...
				if((cpu.registers.cir & 0x0F) == 2) {
					setState(0, "Execute", "The *opcode* 0010 and the *Control Bus* causes the value of the *Accumulator* register to be sent to the output device");
